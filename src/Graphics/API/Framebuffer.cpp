...
Framebuffer::Framebuffer(SDL_GPUDevice* device, uint32_t width, uint32_t height, const std::vector<SDL_GPUTextureFormat>& formats, bool hasDepth, TextureFilter depthFilter)
    : m_Device(device), m_Width(width), m_Height(height)
{
    // 1. Create Color Targets
    for (const auto& format : formats) {
        auto tex = std::make_unique<Texture>(
            m_Device, m_Width, m_Height, format,
    ...
    if (hasDepth) {
        ...
        m_DepthTarget = std::make_unique<Texture>(
            m_Device, m_Width, m_Height, depthFormat,
            SDL_GPU_TEXTUREUSAGE_DEPTH_STENCIL_TARGET | SDL_GPU_TEXTUREUSAGE_SAMPLER,
            depthFilter  // ShadowCompare for shadow maps, Nearest for G-Buffer
        );
    }
